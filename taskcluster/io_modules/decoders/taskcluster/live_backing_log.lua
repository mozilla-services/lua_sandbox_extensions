...
local task_resolution_schema_file = cfg.taskcluster_schema_path .. "/task_resolution.1.schema.json"
fh = assert(io.open(task_resolution_schema_file, "r"))
local task_resolution_schema = fh:read("*a")
task_resolution_schema = rjson.parse_schema(task_resolution_schema)
fh:close()
...
-- taskcluster/treeherder live_backing.log
...
local base_msg   = nil
local function inject_timing_msg(g, level, fields)
...
local function perfherder_decode(g, b, json)
    ...
    j.collection    = base_msg.Fields.collection
    j.framework     = j.framework.name
    j.groupSymbol   = base_msg.Fields.groupSymbol
    j.platform      = base_msg.Fields.platform
    j.project       = base_msg.Fields.project
    j.pushId        = base_msg.Fields.pushId
    j.revision      = base_msg.Fields.revision
    j.symbol        = base_msg.Fields.symbol
    j.taskGroupId   = base_msg.Fields.taskGroupId
    j.taskId        = base_msg.Fields.taskId
    j.tier          = base_msg.Fields.tier
    ...
    if not err then
        local msg = {
            ...
            Payload = payload,
            Fields = {
                symbol      = base_msg.Fields["symbol"],
                groupSymbol = base_msg.Fields["groupSymbol"],
...
local function eval_end_block(g, b, level, cache)
    ...
    if not g.cache_on_completion then
        inject_timing_msg(g, b.level, cache.Fields)
        b.cache[g] = nil
    end
    return true
end
...
local function new_task_completion(g, b, cache)
    ...
    if cache.logStart and cache.logEnd then
        ...
        inject_timing_msg(g, level, f)

        cache.Fields.result = result
        inject_timing_msg(g, level, cache.Fields)
...
local hazard_gen = {
    ...
    Component       = "hazard",
    SubComponent    = nil, -- set by the parser
    Grammar         = bg{"Running %s to generate %s", "subComponent", "_"},
    fn              = eval_line_nm
}
...
local upload = {
    ...
    -- Windows workers output those stats after the task end but Linux workers
...
local function finalize_log(g, b)
    ...
    if #errors > 0 and base_msg.Fields["state"] == "completed" then
        local msg = {
...
local function inject_task_msg(j, name, schema, file)
    local payload = cjson.encode(j)
    doc:parse(payload)
...
function decode(data, dh, mutable)
    ...
    if type(md) == "table" then
        base_msg.Fields["owner"]    = md.owner
    end
    ...
    if type(th) == "table" then
        base_msg.Fields["kind"]         = th.jobKind or tj.tags.kind
        ...
        local key
        if base_msg.Fields["groupSymbol"]:match("^Rap") then
            key = "Rap"
        else
            local np = normalize_platform(tostring(base_msg.Fields["platform"]))
            key = np .. "_" ..  base_msg.Fields["symbol"] .. "_" .. base_msg.Fields["groupSymbol"]
            local name, chunk = base_msg.Fields["symbol"]:match("(.+)-%d+$")
            if name then
                key = np .. "_" .. name .. "_" .. base_msg.Fields["groupSymbol"]
            end
        end
        g = schemas_map[key]
    ...
    -- fetch and parse the live backing log
    ...
    local state = {
        ...
        level           = 1,
        sts             = nil,
        pts             = time:match(run.started),
        pmts            = nil,
        next_match_cb   = nil,
        tc_result       = run.reasonResolved,
        tc_task_result  = nil,
        buffer          = {nil, nil, nil},
        buffer_head     = 0,
        current         = nil,
    ...
    -- Rap-Prof are the main culprits here, but not the only ones without the raptor preformance data in the live_backing.log
    if base_msg.Fields["groupSymbol"]:match("^Rap") and not state.cache.global.raptor_embedded then
        local perfherder_url  = string.format("%s/runs/%d/artifacts/public/test_info/perfherder-data.json", task_url, pj.runId)
        local rv = os.execute(string.format("curl %s -o %s -L -s --compressed", perfherder_url, perfherder_file))
        if rv ~= 0 then return string.format("curl rv: %d, %s", rv, perfherder_url) end
        local fh = assert(io.open(perfherder_file, "rb"))
...
schemas[new_log({new_task({}),})] = {"win_SDR_?","win_Tools_?","other_Certs_?","win_NoPCLMUL_Cipher","win_MPI_?","other_sharedb_SSL","other_WPT_?","win_Lowhash_?","other_WPT_?","win_NoAESNI_Cipher","other_Nightly_?","win_Chains_?","osx_mb_py3","other_WPT-6+_?","other_standard_SSL","other_EC_?","win_EC_?","other_NoAVX_Cipher","other_Gtest_?","win_NoSSSE3|NEON_Cipher","win_pkix_SSL","osx_cargotest_WR","win_NoAVX_Cipher","other_B_?","other_MPI_?","win_Gtest_?","win_SMIME_?","win_Tests-F_FIPS","android_Dev build (macOS)_?","other_Unit_?","other_upgradedb_SSL","other_WPT_?","win_upgradedb_SSL","other_NoPCLMUL_Cipher","win_sharedb_SSL","other_NoAESNI_Cipher","win_Certs-F_FIPS","osx_mb_py2","win_standard_SSL","other_Merge_?","other_NoSSSE3|NEON_Cipher","win_B_FIPS","other_WPT_?","win_B_?","win_Merge_?","other_WPT_?","win_Nightly_?","win_DB_?","win_CRMF_?","win_Policy_?","other_Dev_?","osx_ml_py2","win_Dev build_?","osx_wrench_WR","win_Unit_?","win_Default_Cipher","other_Release_?","win_Certs_?",}
...
schemas[new_log({new_task({new_mozharness({raptor_recording_date,perfherder,}),}),})] = {"Rap",}
...
schemas[new_log({new_task({new_mozharness({}),}),})] = {"android_Jit2_?","android_Jit7_?","android_Jit9_?","osx_TVw_?","android_Jit3_?","android_mda3_M-1proc","android_Jit4_?","win_TVw_?","android_Jit6_?","android_Jit8_?","android_Jit1_?","android_Jit5_?","android_mda2_M-1proc","android_Jit10_?",}
