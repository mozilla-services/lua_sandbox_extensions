...
if read_config("have_zlib") then
    ...
    values = json:find("payload", "values")
    assert(values)
    assert(json:size(json:find("payload")) == 1, json:size())
    local rv = json:remove_shallow("payload", "values")
    assert(json:size(json:find("payload")) == 0, json:size())
    assert(json:size() == 2, json:size())
    assert(2 == json:value(json:find(rv, 1)))
    local f = json:make_field(rv)
    assert(f.userdata == json)
    assert(f.value == rv)
    assert(f.representation == "json")
    ...
    assert(values)
    assert(json:size(json:find("payload")) == 1, json:size())
    local rv = json:remove("payload", "values")
    assert(json:size(json:find("payload")) == 0, json:size())
    assert(json:size() == 2, json:size())
    assert(2 == rv:value(rv:find(1)))
...
ok, doc = pcall(doc.parse_message, doc, hsr, "Payload")
assert(ok, doc)
assert("number" == doc:type(), doc:type())
assert(9 == doc:value(), tostring(doc:value()))
rv = doc:remove_shallow("foo")
assert(not rv)
